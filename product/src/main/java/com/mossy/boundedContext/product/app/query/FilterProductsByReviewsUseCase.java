...
public class FilterProductsByReviewsUseCase {
    ...
    public List<ProductResponse> execute(List<Long> productIds) {
        ...
        // Map에 카테고리 이름 정보가 포함되어 있어야 함
        Map<Long, CatalogReviewInfoDto> reviewInfoMap = catalogProductQueryPort.getReviewInfos(catalogIds);

        return products.stream()
                .map(product -> {
                    CatalogReviewInfoDto info = reviewInfoMap.get(product.getCatalogProductId());
                ...
                .sorted(Comparator.comparing(ProductResponse::reviewCount).reversed())
                .limit(10)
