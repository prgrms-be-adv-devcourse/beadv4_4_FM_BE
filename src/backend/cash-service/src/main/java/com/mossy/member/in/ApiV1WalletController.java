...
//
//import com.mossy.boundedContext.app.CashFacade;
//import com.mossy.boundedContext.app.CashFacade;
...
//    @GetMapping("/user/wallet/logs") // 경로를 기능에 맞게 /logs 추가 권장
//    public RsData<List<com.mossy.shared.cash.dto.response.UserCashLogResponseDto>> getUserCashLogs(
//            @Parameter(hidden = true) @AuthenticationPrincipal UserDetailsImpl userDetails) {
//        Long userId = userDetails.getUserId();
//        List<UserCashLog> userCashLogs = cashFacade.findAllCashLogs(userId);
//
//        // 엔티티 -> DTO 변환
//        List<com.mossy.shared.cash.dto.response.UserCashLogResponseDto> dtos = userCashLogs.stream()
//                .map(com.mossy.shared.cash.dto.response.UserCashLogResponseDto::from)
//                .toList();
//
//        return new RsData<>("C-200", "캐시 내역 조회 성공", dtos);
//    }
//
//    // --- [판매자(Seller) 관련 API] ---
//
