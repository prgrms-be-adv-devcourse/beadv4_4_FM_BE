...
import backend.mossy.boundedContext.payout.domain.PayoutPolicy;
import backend.mossy.shared.member.dto.event.SellerDto;
import backend.mossy.standard.ut.Util;
...
public class PayoutDataInit {
    ...
    public void createTestData() {
        ...
        // 2. 구매자 Seller 생성
        SellerDto buyer = SellerDto.builder()
                .id(100L)
                .userId(1000L)
                ...
                .updatedAt(LocalDateTime.now())
                .build();
        payoutFacade.syncSeller(buyer);
        payoutFacade.createPayout(buyer.id());
        log.info("구매자 Seller 생성 완료: ID={}", buyer.id());
