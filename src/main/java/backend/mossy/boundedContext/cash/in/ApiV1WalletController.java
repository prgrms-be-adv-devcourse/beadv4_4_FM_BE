...
public class ApiV1WalletController {
    ...
    public RsData<List<backend.mossy.shared.cash.dto.response.UserCashLogResponseDto>> getUserCashLogs(
            @Parameter(hidden = true) @AuthenticationPrincipal UserDetailsImpl userDetails) {
        Long userId = userDetails.getUserId();
        List<UserCashLog> userCashLogs = cashFacade.findAllCashLogs(userId);

        // 엔티티 -> DTO 변환
        List<backend.mossy.shared.cash.dto.response.UserCashLogResponseDto> dtos = userCashLogs.stream()
                .map(backend.mossy.shared.cash.dto.response.UserCashLogResponseDto::from)
                .toList();
