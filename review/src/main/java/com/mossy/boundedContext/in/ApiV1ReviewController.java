...
import com.mossy.global.dto.PageResponse;
...
public class ApiV1ReviewController {
    ...
    @GetMapping
    public RsData<PageResponse<ReviewResponse>> getReviewsByProductId(
            @RequestParam Long productId,
            @Parameter(hidden = true)
            @PageableDefault(size = 10) Pageable pageable
    ) {
        Page<ReviewResponse> response = reviewFacade.getReviewsByProductId(productId, pageable);
        return RsData.success(SuccessCode.REVIEW_LIST_GET, new PageResponse<>(response));
    }
    ...
    @GetMapping("/me")
    public RsData<PageResponse<ReviewResponse>> getMyReviews(
            @RequestHeader("X-User-Id") Long userId,
            @Parameter(hidden = true)
            @PageableDefault(size = 10) Pageable pageable
    ) {
